...
int main() {
	...
	for (i = 0; i < NUMBER_OF_TRIALS; i++) {
		...
		float floatp = (float)rand()/(float)(RAND_MAX) * 5;
...
//		REPEAT_40
	...
	for (i = 0; i < NUMBER_OF_TRIALS; i++) {
		...
		sumOfCycles += numClockCycles[i];
	}
	timeAvg = sumOfCycles/NUMBER_OF_TRIALS;
	eventAvg = timeAvg / numEvents;
	xil_printf("\n%d,%d,%d\n",sumOfCycles,timeAvg,eventAvg);
